...
while (OPE)
{
    ...
    case 3: Nuevos_Productos(INV, &TAM);
     ...
     número de elementos del arreglo en la funcion.   */
        break;
...
Se ingresan productos y cantidades, el fin de datos está dado por el cero.
Además de obtener el total de las ventas, se actualiza el inventario.   */
{
int CLA, CAN, I, RES;
float TOT, PAR;
...
while ((*T < 30) && (CLA))
{
    ...
    I++;
    if (I == *T)     /*   Se inserta el elemento en la ultima posición.   */
    {
    A[I].clave = CLA;
    printf("\tNombre:");
    fflush(stdin);
